...
namespace GitHub.Runner.Common.Tests.Listener
{
    public sealed class RunnerL0 : IDisposable
    {
        ...
        [Trait("Level", "L0")]
        [Trait("Category", "Runner")]
        public async Task TestRunAsyncDoesNotCleanupLocalConfigWhenCreateSessionReturnsSessionConflict()
        {
            using (var hc = new TestHostContext(this))
            {
                //Arrange
                ...
                //Assert
                Assert.Equal(Constants.Runner.ReturnCode.SessionConflict, result);
                _messageListener.Verify(x => x.CreateSessionAsync(It.IsAny<CancellationToken>()), Times.Once());
                _messageListener.Verify(x => x.GetNextMessageAsync(It.IsAny<CancellationToken>()), Times.Never());
                _messageListener.Verify(x => x.DeleteSessionAsync(), Times.Never());
                _messageListener.Verify(x => x.DeleteMessageAsync(It.IsAny<TaskAgentMessage>()), Times.Never());
                _configurationManager.Verify(x => x.DeleteLocalRunnerConfig(), Times.Never());
            }
        }
        ...
        [Trait("Level", "L0")]
        [Trait("Category", "Runner")]
        public async Task TestRunAsyncCleanupLocalConfigWhenCreateSessionReturnsRunnerRegistrationDeleted()
        {
            using (var hc = new TestHostContext(this))
            {
                //Arrange
                ...
                _configurationManager.Setup(x => x.IsConfigured())
                    .Returns(true);
                _messageListener.Setup(x => x.CreateSessionAsync(It.IsAny<CancellationToken>()))
                    .Returns(Task.FromResult<CreateSessionResult>(CreateSessionResult.RunnerRegistrationDeleted));
                _configStore.Setup(x => x.IsServiceConfigured()).Returns(false);
